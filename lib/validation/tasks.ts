...
export const createTaskSchema = z.object({
  ...
  project: z.string().max(100).trim().optional().default("General"),
...
export const taskCommentSchema = z.object({
  comment: z.string().min(1, "Comment text is required").max(2000).trim(),
  author: z.string().min(2, "Author is required").max(80).trim().default("WDS Member"),
  type: z.enum(["NOTE", "BLOCKER", "HANDOVER", "TECHNICAL", "GENERAL"]).default("NOTE"),
});
